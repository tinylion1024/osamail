...
function run(argv) {
    try {
        ...
        if (
            request.operation !== "open_message" ||
            !request.locator ||
        ...
        var mailbox = findMailbox(account, request.locator.mailbox_path);
        var messages = mailbox.messages.whose({
            id: Number(request.locator.message_id),
        })();
        if (messages.length === 0) {
            throw new Error("OSAMAIL_MESSAGE_NOT_FOUND");
        }
        if (
            request.locator.internet_message_id &&
            String(messages[0].messageId()) !==
                String(request.locator.internet_message_id)
        ) {
            throw new Error("OSAMAIL_MESSAGE_NOT_FOUND");
        }
